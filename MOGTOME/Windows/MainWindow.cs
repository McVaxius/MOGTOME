...
public class MainWindow : Window, IDisposable
{
    ...
    public override void Draw()
    {
        ...
        if (plugin.DeathTrackingService.IsActive)
        {
            ...
            ImGui.Text($"Deaths this run: Self {deaths.SelfDeathCount} | Others {deaths.OtherDeathCount} | All {deaths.TotalDeathCount}");
        }
        ...
        DrawStatusLine("Food", config.FoodItemId > 0 && state.FoodAvailable, FormatConsumableLabel(config.FoodItemName, config.FoodUseHighQuality, state.FoodExactCount));
    ...
    private static string FormatConsumableLabel(string name, bool highQuality, int exactCount)
    {
        ...
        return $"{name} [{(highQuality ? "HQ" : "NQ")}] x{Math.Max(0, exactCount)}";
    }
}
